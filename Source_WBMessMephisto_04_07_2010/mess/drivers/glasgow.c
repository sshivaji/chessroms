...
//logerror("Keyboard Offset = %x Data = %x\n  ",offset,data);
  ...
  if (key_select==key_hi)
  {
	...
	if (data!=0x300 && (g_displayChanged  || 			    //MOD RS Rückmeldung Tastendruck erkannt (Display hat sich geändert)
...
static READ16_HANDLER(read_newkeys16)  //Amsterdam, Roma
{
 ...
 if (key_selector==0) 
	...
	if (data && (g_displayChanged || 						//MOD RS Rückmeldung Tastendruck erkannt (Display hat sich geändert)
		               (g_waitCnt > g_inputTimeout ) ) )	//MOD RS Timeout warten auf Rückmeldung (z.B. beim Info Befehl zur Abfrage Promo
														    //MOD RS oder LEV 
	{
		g_portIsReady=TRUE;									//MOD RS
	...
	{										//MOD RS
		...
		{									//MOD RS
			...
			else									//MOD RS	
			{
				...
				if (sendBM_repeat>0)				//MOD RS			-> Prüfen ob das überhaupt notwendig ist !!!
				{									//MOD RS
