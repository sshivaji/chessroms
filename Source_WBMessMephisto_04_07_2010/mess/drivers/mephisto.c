...
// 19.07.2009 MM2 hinzugefügt
// Folgende Probleme: 
//
// NMI Timer darf erst loslaufen, wenn bestimmte Intitialisierungen gemacht wurden. 
// Welche das sind ist unklar. Problem durch eine 2. Timerroutine umgeangen, started wird gesetzt, NMI erst wenn started

// NMI läuft nicht mit der vollen Frequenz. Uhr ist zu langsam. Bei voller Frequenz funktioniert die Emulation nicht mehr
// Problem durch höhere Taktfrequent umgangen. Im MM2 Modul befindet sich ein 7.3 MHZ Quarz, der Prozessor ist aber nur
// mit 3.7 MHZ getackted ???
//

// Rom des Eröffnungsmoduls nicht eingebaut.
//
...
static int sendBM_delay=0;				//MOD RS	//Zähler Wartezeit Bis Besmove gesendet werden kann
...
static WRITE8_HANDLER ( write_lcd )
{
	if (led7==0)
	{
		...
		output_set_digit_value(lcd_shift_counter,data);    // 0x109 MM IV // 0x040 MM V

		assert(lcd_shift_counter<=3);					//MOD RS
		assert(data<=128);								//MOD RS
	
		g_display[lcd_shift_counter]=g_segment[data];	//MOD RS
			...
			strncmp(g_display,saveDisplay,4) )			//MOD RS
		{
			...
			//if (!(g_state==SEARCHING	&&				//MOD RS		Keine Displayausgabe während der Suche
...
//			printf("g_dispayChanged\n");				//MOD RS
			...
			if (strncmp(g_display,"TIME",4))			//MOD RS Sonderfall Anzeige TIME soll nicht g_displayChanged auslösen
...
static READ8_HANDLER(read_keys)
{
	...
	data = 0xff;
	if (((led_status & 0x80) == 0x00))									//Bit 7 = LED 7 = 0
		strcpy(keystring,keynames[0][offset]);			//MOD RS
	else																//Bit 7 = LED 7 = 1
		strcpy(keystring,keynames[1][offset]);			//MOD RS

	data=input_port_read(space->machine, keystring);	//MOD RS		

	if (data==0 && (g_displayChanged  || 			    //MOD RS Rückmeldung Tastendruck erkannt (Display hat sich geändert)
		           (g_waitCnt > g_inputTimeout ) ) )	//MOD RS Timeout warten auf Rückmeldung (z.B. beim Info Befehl zur Abfrage Promo
														//MOD RS oder LEV 
	{
		g_portIsReady=TRUE;
		input_port_clear(space->machine,keystring);		//MOD RS
	}
...
// Unbekannte Eingabe beim rebel5
//
static READ8_HANDLER (read_unknown_rebel5)				//MOD RS
{
	UINT8 data;											//MOD RS

	//Log("read_unknown_rebel5 !!!\n");					//MOD RS
	//data=input_port_read(space->machine, "KEY1_5");	//MOD RS Taste ENT
	//Log("data: %x\n",data);							//MOD RS
	data=0x00;											//MOD RS

	return data | 0x7f;									//MOD RS
}														//MOD RS
...
static WRITE8_HANDLER ( write_led )
{
	...
	if (offset==7) 
		led7= data & 0x80 ? 0x00 :0xff;

	if (offset==6 && g_state==SEARCHING)						//MOD RS	Warten wenn Suche beendet wird
		sendBM=TRUE;											//MOD RS    Bestmove wird in timer routine gesendet

	//Log("offset: %d data: %d Display: %s state: %s\n",offset,data,g_display,PrintState(g_state));
...
static ADDRESS_MAP_START(rebel5_mem , ADDRESS_SPACE_PROGRAM, 8)
	...
	AM_RANGE( 0x3000, 0x3007) AM_READ( read_keys )	// Rebell 5.0
	AM_RANGE( 0x2000, 0x2007) AM_WRITE( write_led )	// Status LEDs+ buzzer

	AM_RANGE( 0x3000, 0x4000) AM_READ( read_board)			//Chessboard		reeds

	AM_RANGE( 0x4002, 0x4002) AM_READ( read_unknown_rebel5)	// Unbekannte Eingabe
...
static ADDRESS_MAP_START(mm2_mem , ADDRESS_SPACE_PROGRAM, 8)
	...
	AM_RANGE( 0x2800, 0x2800) AM_WRITE( write_lcd )				//MM2 ok
	AM_RANGE( 0x1800, 0x1807) AM_READ( read_keys )				//MM2 ok	
	AM_RANGE( 0x1000, 0x1007) AM_WRITE( write_led_mm2 )			//MM2 ok Status LEDs

	AM_RANGE( 0x3000, 0x3000) AM_WRITE ( write_board_mm )		//MM2 ok Chessboard		Set LED's  

	AM_RANGE( 0x3800, 0x3800) AM_WRITE ( write_board_mask)		//MM2 ok Chessboard	    mask

	AM_RANGE( 0x2000, 0x2000) AM_READ( read_board )				//MM2 ok Chessboard		 reeds	
...
{											//MOD RS
	g_waitCnt++;							//MOD RS   Zähler für Verzögerung Eingabe
}											//MOD RS


static TIMER_CALLBACK( BM_Check )			//MOD RS
{
	if (sendBM && g_state==SEARCHING)		//MOD RS   Verzögerung bei Ausgabe bestmove
	{										//MOD RS
//		Log("sendBM -> sendBM_delay: %d\n",sendBM_delay);
...
static TIMER_CALLBACK( update_started )					//only MM2 - there must be an other solution 
{
	started=TRUE;
}
...
static MACHINE_START( mm2 )
{
	...
	timer_pulse(machine, ATTOTIME_IN_HZ(380), NULL, 0, update_nmi);			// MM2 test
	timer_pulse(machine, ATTOTIME_IN_HZ(5), NULL, 0, update_started);
	timer_pulse(machine, ATTOTIME_IN_HZ(20), NULL, 0, update_artwork);

	beep_set_frequency(speaker, 3500);
	...
	started=FALSE;		// process timer update_nmi 
}
...
static MACHINE_START( mephisto )
{
	...
	timer_pulse(machine, ATTOTIME_IN_HZ(600), NULL, 0, update_nmi);			//Orginal
...
//	timer_pulse(machine, ATTOTIME_IN_HZ(75), NULL, 0, update_artwork);		//18 MHZ
	timer_pulse(machine, ATTOTIME_IN_HZ(60), NULL, 0, update_waitCnt);		//MOD RS           //Zähler für Verzögerung Eingabe
	...
	started=TRUE;		// process timer update_nmi 
...
//
	 ...
	 //clear_layout();					//MOD RS erstmal raus vieleich über Defines steueren
...
static MACHINE_DRIVER_START( mephisto )
	/* basic machine hardware */
																						//MODRS
	MDRV_CPU_ADD("maincpu",M65C02,4915200)        /* 65C02 */																							//MODRS
...
static MACHINE_DRIVER_START( mm2 )
	MDRV_IMPORT_FROM( mephisto )

//	MDRV_CPU_REPLACE("maincpu", M65C02, 3700000)
	MDRV_CPU_REPLACE("maincpu", M65C02, 7372800)
//	MDRV_CPU_MODIFY("maincpu")
//	MDRV_CPU_CLOCK(3700000)
...
ROM_START(mm4)
	ROM_REGION(0x10000,"maincpu",0)
	ROM_LOAD("mm4.bin", 0x8000, 0x8000, CRC(f68a4124) SHA1(d1d03a9aacc291d5cb720d2ee2a209eeba13a36c))
...
CONS( 1990, mm5,        0,      0,      mephisto,   mephisto,   mephisto,   "Hegener & Glaser", "Mephisto 5.1 Schach Computer",		GAME_SUPPORTS_SAVE )
CONS( 1990, mm50,       0,      0,      mephisto,   mephisto,   mephisto,   "Hegener & Glaser", "Mephisto 5.0 Schach Computer",		GAME_SUPPORTS_SAVE )
CONS( 1986, rebel5,     0,      0,      rebel5,     mephisto,   mephisto,   "Hegener & Glaser", "Mephisto Rebel 5 Schach Computer", GAME_SUPPORTS_SAVE )
CONS( 1984, mm2,        0,      0,      mm2,        mephisto,   mephisto,   "Hegener & Glaser", "Mephisto MM2 Schach Computer",		GAME_SUPPORTS_SAVE )
...
static int loadFENfile(char *buffer)
{
	...
	{
//		popmessage("File fen.txt not found"); //MOD RS
		return FALSE;
	}
