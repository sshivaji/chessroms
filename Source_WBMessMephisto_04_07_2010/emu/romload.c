...
static int open_rom_file(rom_load_data *romdata, const char *regiontag, const rom_entry *romp)
{
	...
	//display_loading_rom_message(romdata, ROM_GETNAME(romp));	//MOD RS
...
//	display_rom_load_results(romdata);  //MOD RS
}
#endif
...
//	display_rom_load_results(romdata);		//MOD RS
	...
	if (romdata->errors > 0)											        		//MOD RS
	{																					//MOD RS
		PrintAndLog("Error loading roms: \n%s\n",astring_c(&romdata->errorstring) );		//MOD RS
		astring_free(&romdata->errorstring);												//MOD RS
		fatalerror_exitcode(romdata->machine, MAMERR_MISSING_FILES, "ERROR: required files are missing, the "GAMENOUN" cannot be run."); //MOD RS
	}																					//MOD RS
