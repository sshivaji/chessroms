...
static void frame_update_callback(running_machine *machine)
{
	...
	//frame_update(machine);				//MOD RS auskommetieren ? Port werden hier gelöscht !
}
...
static int frame_get_digital_field_state(const input_field_config *field, int mouse_down)
{
	...
	//if (field->type == IPT_KEYBOARD && ui_get_use_natural_keyboard(field->port->machine)) //MOD RS
	//	return FALSE;																		//MOD RS
...
// input_port_set
//------------------------------
void input_port_set(running_machine *machine, const char *tag, int data)
{
	input_port_config *port = (input_port_config*)machine->port(tag);;
	if (port == NULL)
		fatalerror("Unable to locate input port '%s'", tag);
	else
		port->state->digital=data;
...
// input_port_clear
//------------------------------
void input_port_clear(running_machine *machine, const char *tag)
{
	input_port_config *port = (input_port_config*)machine->port(tag);;
	if (port == NULL)
		fatalerror("Unable to locate input port '%s'", tag);
